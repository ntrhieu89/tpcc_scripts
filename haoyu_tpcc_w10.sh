						...
						do
										...
										do
											...
											do
												...
												if [[ $persistMode == "async_bdb" ]]; then
													cmd="nohup $cache_dir/src/twemcache -q 1 -i /mnt/bdb-$persistMode-$ip-$port-$time -w $warehouses -F 1 -t $threadsPerCMI -c 8192 -m $cachesize -g 7000 -G 999999 -p $port"
												elif [[ $persistMode == "sync_bdb" ]]; then
													cmd="nohup $cache_dir/src/twemcache -q 1 -Q 1 -i /mnt/bdb-$persistMode-$ip-$port-$time -w $warehouses -F 1 -t $threadsPerCMI -c 8192 -m $cachesize -g 7000 -G 999999 -p $port"
												else
						    ...
						    #                     echo "waiting for $ip $i"
							   ...
							   #      done
								    ...
								    cp $bench/results/* $dir
									rm -r $bench/results/*

							        # Copy the files over local node
							        dir="bdb-$cache-try-$try-w-$warehouses-ar-$ar-th-$threads-tpc-$threadsPerCMI-cps-$cacheperserver-pm-$persistMode-rep-$rep-cz-$cachesize-evict-$eviction"
