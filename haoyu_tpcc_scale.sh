...
for try in 1 #2 3 4
do
for warehouses in 20 30 40 50 60 70 80
do
cacheips=( "h11" "h12" "h13" "h14" "h15" "h16" "h17" "h18" "h19" "h20" )
clis=( "h1" "h2" "h3" "h4" "h5" "h6" "h7" "h8" "h9" "h10" )
			...
			do
				...
				do
					...
					do
						...
						do
							for persistMode in "no_persist" #"sync_bdb"
							do
										...
										do
											...
											do
												...
												if [[ $persistMode == "async_bdb" ]]; then
													cmd="nohup $cache_dir/src/twemcache -q 1 -i /mnt/bdb-$persistMode-$ip-$port-$time -w 100 -F 1 -t $threadsPerCMI -c 8192 -m $cachesize -g 7000 -G 999999 -p $port -Z 31"
