...
dbip="h0"
cacheips=( "h9" "h10" "h11" "h12" "h13" "h14" "h15" "h16" "h17" "h18" )
#cacheips=( "h9" )
cacheperserver="10"
...
for try in 1
do
for threads in 200 400 600 800 1000
do
ar=$threads
#ar=0
for batch in 1
...
do
	...
	if [[ $copydb == "true" ]]; then
		...
		sudo cp -av /mnt2/mysql/* /mnt/mysql
	...
	if [ $cache == "true" ]; then
		...
		do
			...
			for ((i=0; i < $cacheperserver; i++))
			do
				ssh -oStrictHostKeyChecking=no $ip "nohup $base/IQ-Twemcached/src/twemcache -t 8 -c 8192 -m 10000 -g 7000 -G 999999 -p $port > $dir/cache$ip.txt &" &
				port=$((port+1))
			done
		done
