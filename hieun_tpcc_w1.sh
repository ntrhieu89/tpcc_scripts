		...
		do
			...
			do
				...
				if [[ $persistMode == "sync_bdb" ]]; then
					cmd="$cahce_dir/src/twemcache -q 1 -Q 1 -i /mnt/bdb-$persistMode-$ip-$port-$time -w 1 -F 1 -t $threadsPerCMI -c 12 -m $cachesize -g 7000 -G 999999 -p $port"
				elif [[ $persistMode == "async_bdb" ]]; then
					cmd="$cahce_dir/src/twemcache -q 1 -i /mnt/bdb-$persistMode-$ip-$port-$time -w 1 -F 1 -t $threadsPerCMI -c 12 -m $cachesize -g 7000 -G 999999 -p $port -Z 31"
				else
					cmd="$cahce_dir/src/twemcache -t $threadsPerCMI -c 12 -m $cachesize -g 7000 -G 999999 -p $port"
				fi
